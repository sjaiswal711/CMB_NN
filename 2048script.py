...
def process_time_step(time_step):
    ...
    neighbours = hp.query_disc(2*nside, R , radius=(3*sigma))
    ...
    for i, neighbour_pix in enumerate(neighbours):
        R_i = hp.pix2vec(2*nside,neighbour_pix,nest=False)
...
temperature_map = hp.read_map("input_map16.fits")
...
chunks = np.array_split(time_periods, 1)
